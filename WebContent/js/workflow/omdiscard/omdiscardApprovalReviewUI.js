				...
				},
				{
					...
					items : [ {
						...
						handler : function(grid, rowIndex, colIndex) {
							window
									.open("../workflow/printpage_084_1E.html?taskId="
											+ taskId);
						}
					} ]
				} ],
		...
		items : [
				...
				},
				{
					...
					items : [ {
						...
						anchor : '85%',
						xtype : 'label',
						html : '<div style="padding-top:3px">' + taskno
								+ '</div>'
					} ]
				},
				{
					...
					items : [ {
						...
						anchor : '85%',
						xtype : 'label',
						html : '<div style="padding-top:3px">' + devname
								+ '</div>'
					} ]
				},
				{
					...
					items : [ {
						...
						html : '<div style="padding-top:3px">'
								+ omDiscardProcess_discardTypeDispaly(discardType)
								+ '</div>'
					} ]
				}, {
					clumnWidth : 1,
...
function omDiscardApproval_handler() {
	...
	if (!Ext.getCmp("checkResult_id").isValid()
			|| !Ext.getCmp("trialNumber_id").isValid()) {
		return;
	}

	var _reviewResultValue = Ext.getCmp("checkResult_id").getValue();
	var _reviewNoValue = Ext.getCmp("trialNumber_id").getValue();

	var _devno;
	for (var i = 0; i < rs.length; i++) {
		rs[i].set("REVIEWRST", _reviewResultValue);
		rs[i].set("REVIEWNO", _reviewNoValue);
		rs[i].commit();
	...
	Ext.Ajax.request({
		...
		params : {
			flowId : flowId,
			devNo : _devno,
			reviewRst : _reviewResultValue,
			reviewNo : _reviewNoValue,
			checkFlag : 2
		},
		method : 'POST',
		success : function(response) {
