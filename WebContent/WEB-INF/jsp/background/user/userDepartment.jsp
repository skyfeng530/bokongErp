...
<html>
  ...
  function allocation(){
       $.ajax({
      		...
      		type : "POST",
      		data:{userName:"${user.userName}",departId:$('input[name="departId"]:checked').val()},
      		url: "${pageContext.servletContext.contextPath }/background/user/allocationDepartment.html",
      		dataType:'json',
...
<br/>  
      ...
      <tr>
        ...
        <td><table class="ttab" width="100%" cellspacing="1" onmouseover="changeto()"  onmouseout="changeback()">
          ...
          <c:forEach var="dm" items="${departments}">
          <tr>
            <td height="20" >
              <input type="radio" name="departId" value="${dm.departId}" />
            </td>
            <td height="20" ><span class="STYLE1">${dm.dName}</span></td>
            <td height="20" ><span class="STYLE1">
             <c:if test="${dm.departType eq '0'}">
            <font color="red">虚拟部门</font>
            </c:if>
            <c:if test="${dm.departType eq '1'}">
